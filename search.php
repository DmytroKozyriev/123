...
define('DB_USER', 'kamenshc_form1');
define('DB_PASS', 'Kozak33708');
define('DB_NAME', 'kamenshc_form');
    ...
    { 
        if (strlen($query) < 3) {
            ...
            $q = "SELECT `page_id`, `title`, `desc`, `title_link`, `category`, `uniq_id`
                  FROM `form1` WHERE `text` LIKE '%$query%'
            ...
            if (mysql_affected_rows() > 0) { 
                ...
                do {
                    ...
                    $q1 = "SELECT `link` FROM `form1` WHERE `uniq_id` = '$row[page_id]'";
